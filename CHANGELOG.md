...
### Added
- Runtime card **Closed-loop AI control** (`host ask` / `flow://`): drive a NAT'd node from natural language — the LLM plans a YAML flow over the node's live routes, runs it, and self-repairs from the node's own errors (EN/PL/DE).
